	...
	for (size_t i = 0; i < Buttons.size(); i++)
	{
		...
		for (auto& btn : Buttons.at(i).Buttons)
		{
			...
			ButtonBackground->AddChild((new UIBackground(false, 0, UIColors[3], Vector2(0.0f)))
				...
				->AddChild((new UIButton(true, 0, UIColors[2], this, (int)i * MAX_CATEGORY_BUTTONS + (int)j))
					->SetUseTexture(true, btn.Texture)
					->SetMinSize(0.055f)
					->SetPadding(0.01f, 0.01f, 0.02f, 0.02f)
					->SetSizeMode(UIBox::SizeMode::PixelRelative))
				->AddChild((new UIText(0.375f, UIColors[2], btn.Name, Editor::CurrentUI->EngineUIText))
					->SetPadding(0.005f)));
			j++;
		}
	}
