...
float UIScrollBox::GetDesiredChildrenSize()
{
	if (GetOrientation() == UIBox::Orientation::Vertical)
	{
		...
		{
			DesiredSize += i->UpPadding + i->DownPadding + std::max(i->GetUsedSize().Y, 0.0f);
		}
		return DesiredSize;
	}
	float DesiredSize = 0;
	for (UIBox* i : Children)
	{
		DesiredSize = std::max(i->UpPadding + i->DownPadding + std::max(i->GetUsedSize().Y, 0.0f), DesiredSize);
	}
	return DesiredSize;
}
